...
def assess_risk(
    ...
    """【路径A第1步】5因子风险测评 → 确定风险等级 + 建议配置比例。

    WHEN TO USE:
    - 新用户首次建立投资组合（路径A的起点）
    - 用户询问"我适合什么风险等级"、"帮我做风险测评"
    - 用户描述了自己的投资期限、亏损容忍度、收入、经验、流动性需求时

    HOW TO USE:
    - 从对话中推断参数: horizon(投资期限), max_loss_pct(最大可接受亏损%),
      income(收入稳定性1-5), experience(投资经验1-5), liquidity(流动性需求1-5)
